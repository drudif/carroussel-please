...
A `um-carrossel-por-favor` é **autossuficiente**: nada dentro dela depende de outra skill estar instalada. Isso é decisão de arquitetura, não conveniência — a promessa é `git clone` e funciona, e uma chamada em tempo de execução para uma skill ausente falha em silêncio na máquina de quem instalou.
